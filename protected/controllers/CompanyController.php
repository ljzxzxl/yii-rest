...
class CompanyController extends Controller
{
     ...
     * @return void
     */
    public function _addCompany($model = '')
    {
        if(!empty($model)){
			...
			if(is_array($_POST)){
				...
				// Find the company by company_name
				$company_info=Company::model()->find('company_name=:company_name', array(':company_name'=>$company_name));
				if(!empty($company_info)){
					$this->_sendResponse(501, sprintf('Error: Parameter company_name already exists') );
					return;
				}
			}else{
