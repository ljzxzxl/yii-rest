...
class OrderController extends Controller
{
     ...
     * @return void
     */
    public function _addOrder($model = '')
    {
        if(!empty($model)){
			...
			if(is_array($_POST)){
				...
				// Find the order by order_name
				$order_info=Order::model()->find('order_name=:order_name', array(':order_name'=>$order_name));
				if(!empty($order_info)){
					$this->_sendResponse(501, sprintf('Error: Parameter order_name already exists') );
					return;
				}
			}else{
