...
class FolderController extends Controller
{
     ...
     * @return void
     */
    public function _addFolder($model = '')
    {
        if(!empty($model)){
			...
			if(is_array($_POST)){
				...
				// Find the folder by folder_name
				$folder_info=Folder::model()->find('folder_name=:folder_name', array(':folder_name'=>$folder_name));
				if(!empty($folder_info)){
					$this->_sendResponse(501, sprintf('Error: Parameter folder_name already exists') );
					return;
				}
			}else{
