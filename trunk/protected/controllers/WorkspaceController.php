...
class WorkspaceController extends Controller
{
    ...
    public function actionGet()
    {
        ...
        switch($_GET['model'])
        {
            ...
            case 'view': // {{{ 
                $model = Workspace::model()->findByPk($_GET['id']);
                break; // }}} 
			case 'user': // {{{ 
     ...
     * @return void
     */
    public function _addWorkspace($model = '')
    {
        if(!empty($model)){
			...
			if(is_array($_POST)){
				...
				// Find the workspace by workspace_name
				$workspace_info=Workspace::model()->find('workspace_name=:workspace_name', array(':workspace_name'=>$workspace_name));
				if(!empty($workspace_info)){
					$this->_sendResponse(501, sprintf('Error: Parameter workspace_name already exists') );
					return;
				}
			}else{
