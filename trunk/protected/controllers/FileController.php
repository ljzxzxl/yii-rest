...
class FileController extends Controller
{
    ...
    public function actionDelete()
    {
		...
		$recycle->user_id = $file['owner_uid'];
		$recycle->obj_id = $file['file_id'];
		$recycle->obj_type = 'file';
     ...
     * @return void
     */
    public function _addFile($model = '')
    {
        if(!empty($model)){
			...
			if(is_array($_POST)){
				...
				$file_info=File::model()->find('file_name=:file_name', array(':file_name'=>$file_name));
				if(!empty($file_info)){
					$this->_sendResponse(501, sprintf('Error: Parameter file_name already exists') );
					return;
				}
			}else{
